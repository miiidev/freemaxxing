...
describe("streaming", () => {
  it("streams SSE with model rewritten and served-by header", async () => {
    ...
    expect(headers["x-freemaxxing-served-by"]).toMatch(/^groq::/);
    expect(body).toContain('"model":"groq::');
    expect(body).not.toContain('"model":"up"');
    expect(body.trimEnd().endsWith("data: [DONE]")).toBe(true);
  });

  it("emits freemaxxing_error frame and never mixes models when upstream dies mid-stream", async () => {
    const fetchImpl = (async () =>
    ...
    expect(body).toContain('"freemaxxing_error":"upstream_stream_failed"');
...
describe("streaming usage recording", () => {
  ...
  it("records on the annotated path too", async () => {
    ...
    expect(res.body).toContain("freemaxxing: "); // annotation still flows through capture untouched
  });
});

describe("streaming tool-call guard", () => {
  it("appends freemaxxing_error frame and logs the event for truncated tool args", async () => {
    ...
    expect(res.statusCode).toBe(200);
    expect(res.body).toContain('"freemaxxing_error":"malformed_tool_call"');
    expect(res.body.indexOf('"freemaxxing_error":"malformed_tool_call"'))
  ...
  it("clean streams stay byte-identical", async () => {
    ...
    expect(res.body).not.toContain("freemaxxing_error");
