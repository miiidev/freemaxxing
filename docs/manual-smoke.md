...
Prereq: valid keys for all six providers exported in the shell or ~/.freemaxxing/.env.

1. `npm run build && node dist/cli.js serve`
   - Expect banner: `freemaxxing serving 6/6 providers on http://127.0.0.1:8787/v1`
...
3. Non-streaming happy path:
   ...
   - Expect 200 JSON, `model` starts with `<provider>::`, header `x-freemaxxing-served-by` present.
...
1. Temporarily hide any real keys: `$env:GROQ_API_KEY=$null` etc.; ensure `%USERPROFILE%\.freemaxxing\.env` does not exist.
...
6. Expect HTTP 200, `x-freemaxxing-served-by` starting `groq::`.
